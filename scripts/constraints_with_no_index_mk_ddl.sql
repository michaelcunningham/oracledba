...
declare
	...
	s_prev_tab	dba_objects.object_name%type;
	s_table_name	dba_objects.object_name%type;
	s_ddl		varchar2(500);
	n_index_number	integer;
...
begin
	...
	dbms_output.put_line( 'The following is a list of indexes that should be created.' );
	dbms_output.put_line( '	' );
	dbms_output.put_line( 'Index DDL' );
	dbms_output.put_line( '----------------------------------------------------------------------------------------------------------------------------------------------------------------' );
...
--		and	constraint_type in( 'R' ) )
	...
	loop
		--
		-- Find the highest number index used on this table.
		--
		if nvl( s_prev_tab, 'X' ) <> r.table_name then
			select	nvl( max( regexp_replace( index_name, '[[:alpha:]_]' ) ), 0 )
			into	n_index_number
			from	dba_indexes
			where	owner = s_owner
		...
		fetch cur_constraint into r_cons;
			...
			if cur_index%notfound then
				--
				-- Check to see if the parent table is a lookup table.
				-- Only continue if it is NOT a lookup table.
				--
				--dbms_output.put_line( r_cons.constraint_name );
				select	table_name
				into	s_table_name
				from	dba_constraints
				where	owner = s_owner
				and	constraint_name = (
                				...
                				from    dba_constraints
                				where   owner = s_owner
						and	constraint_name = r_cons.constraint_name );

				if s_table_name not like 'LU%' then
					--dbms_output.put_line( r_cons.table_name || ' ' || r_cons.constraint_name );
					n_index_number := n_index_number + 1;

					s_ddl := 'create index '
						|| regexp_replace( substr( 'xie' || n_index_number || lower( r_cons.table_name ), 1, 30 ), '_$', null )
						|| ' on ' || lower( r_cons.table_name ) || '( '
						|| lower( rtrim( getstr( r_cons.cname1 )
