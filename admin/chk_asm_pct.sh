...
begin
	for r in(
		...
		where	state = 'MOUNTED' and name != 'FRA'
		)
	loop
		if r.total_bytes <= n_GB * 300 then
			n_pct_free_threshold := 10.0;
			n_mb_free_threshold := 30 * 1024;
		elsif r.total_bytes <= n_TB * 1 then
			n_pct_free_threshold := 10.0;
