...
/mnt/dba/admin/create_to_utildb_data_link.sh $ORACLE_SID
...
begin
	...
	for r in (
		...
		from	dba_db_links
		where	upper( db_link ) not like 'TO_UTILDB_DATA%' )
	loop
		delete	from db_link@to_utildb_data
		where	instance_name = r.instance_name
		and	last_updated < sysdate - 2;

		update	db_link@to_utildb_data
		...
		if sql%notfound then
			insert into db_link@to_utildb_data(
