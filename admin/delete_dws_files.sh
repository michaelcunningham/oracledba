...
rsh npnetapp104 qtree security /vol/docutest1/$ORACLE_SID unix

echo "Exporting filesystem." >> $log_file
rsh npnetapp104 exportfs -p rw,root=$server_name /vol/docutest1/$ORACLE_SID

echo "Mounting filesystem "/tmp/$tmpdoc_mount >> $log_file
sudo mount npnetapp104:/vol/docutest1/$ORACLE_SID /tmp/$tmpdoc_mount
...
rsh npnetapp104 qtree security /vol/docutest1/$ORACLE_SID mixed

find /tmp/$tmpdoc_mount/ -type f -exec rm -f {} \;

sudo umount /tmp/$tmpdoc_mount
rmdir /tmp/$tmpdoc_mount
rsh npnetapp104 exportfs -u /vol/docutest1/$ORACLE_SID
