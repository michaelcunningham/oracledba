...
cmdfile=/mnt/dba/rcv/${ORACLE_SID}_backup_database.rcv
if [ ! -f $cmdfile ]; then
   echo "Unable to find $cmdfile.";
   exit 1;
fi

LOG_LOC=/mnt/db_transfer/${ORACLE_SID}/logs
...
rman target / cmdfile=$cmdfile log=$logName > /dev/null 2>&1
