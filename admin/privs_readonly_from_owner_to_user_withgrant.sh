...
declare
	cursor cur_tab( s_obj_owner varchar2, s_grant_to varchar2 ) is
		select	'grant select on ' || s_obj_owner || '.' || table_name || ' to ' || s_grant_to
				|| ' with grant option' sql_text
		from	dba_tables
		where	owner = s_obj_owner
		and	table_name not in( select table_name from dba_external_tables where owner = s_obj_owner )
		union
		select	'grant select on ' || s_obj_owner || '.' || object_name || ' to ' || s_grant_to
				|| ' with grant option' sql_text
		from	dba_objects
		where	owner = s_obj_owner
	...
	cursor cur_seq_mat( s_obj_owner varchar2, s_grant_to varchar2 ) is
		select  distinct 'grant select on ' || s_obj_owner || '.' || object_name || ' to ' || s_grant_to
				|| ' with grant option' sql_text
		from    dba_objects
		where   owner = s_obj_owner
	...
	cursor cur_syn( s_obj_owner varchar2, s_grant_to varchar2 ) is
		select	'create or replace synonym ' || s_grant_to || '.'
				|| table_name || ' for ' || s_obj_owner || '.' || table_name sql_text
		from    (
			select	table_name
			from	dba_tables
			where	owner = s_obj_owner
