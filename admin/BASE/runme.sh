...
echo "Run one script against one db, one script against each partition(xx script) or one script per pdb?"
...
echo "Run the script in dev, stage or prod?"
...
if [ $execution != "eachpdb" ]; then
	echo "What type of DB do you want to run the script in?"
...
if [ "$db" = "tdbs" ]; then
	...
	echo "Serial or parallel execution? Caution - parallel execution is more expensive"
	select sp in serial parallel; do
		if [[ -n $sp ]]; then
			break
		else
			echo "enter 1=serial 2=parallel"
		fi
	done
	if [ $sp = "serial" ]; then
