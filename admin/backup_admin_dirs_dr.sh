...
################################################################################
#
# This file is inteded to run on the DR Linux server so it backup the admin
# directories back to the Napa office.  It should be run as a subscript inside
# the /dba/copy_from_napa.sh script because that script will mount the /napadba
# filesystem.  That is where we want to copy files back to.
################################################################################
...
do
  backup_dir_name=/napadba/admin_backup/${node_name}_${this_dir}_${log_date}
