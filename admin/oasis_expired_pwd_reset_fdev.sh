...
log_file=$log_dir/oasis_expired_pwd_reset_fdev_$log_date.log
...
then
  echo
  echo "	The username "$username" does not exist in the fdev database."
  echo
  exit
fi
...
alter user $username identified by welcome1;
