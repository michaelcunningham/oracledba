...
if [ $2 -eq 0 ] 
 then
 cmdfile=$jobdir/rman_backup_database_inc0.rcv
 jobName=backup_database_0_$ORACLE_SID
elif [ $2 -eq 1 ]
 then
 cmdfile=$jobdir/rman_backup_database_inc1.rcv
...
rman catalog rman/rman2@rman11 target / cmdfile=$cmdfile log=$logName
