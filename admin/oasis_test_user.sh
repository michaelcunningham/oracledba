...
# so let's find out information about the user.
sqlplus -s /nolog << EOF
connect sys/$syspwd@$tns as sysdba

column username        format a15  heading Username
column account_status  format a15  heading 'Acct Status'
column profile         format a15  heading Profile
column lock_date       format date heading 'Lock Date'
column last_date       format date heading 'Last Date'
alter session set nls_date_format='dd-mon-yyyy hh24:mi';

select username, account_status, profile, lock_date from dba_users where username like '%$username%';
