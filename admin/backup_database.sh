...
# Make sure this directory exists. If not, try to find one that ends in backup
if [ ! -d $backup_dir ]
then
  subdir_name=`ls -1 /mnt | grep backup$ | grep -v dbbackup`
  backup_dir=/mnt/$subdir_name
  if [ ! -d $backup_dir ]
  then
    # We still can't determine a backup_dir, so exit.
    echo
...
then
  ...
  echo "CONFIGURE CONTROLFILE AUTOBACKUP FORMAT FOR DEVICE TYPE DISK TO '/mnt/oralogs/$ORACLE_SID/%F';" >> $cmdfile_name
  ...
  echo "ALLOCATE CHANNEL ch2 DEVICE TYPE DISK FORMAT '$backup_dir/db_%d_df_t%t_s%s_p%p_level${backup_level}';" >> $cmdfile_name
# echo "ALLOCATE CHANNEL ch3 DEVICE TYPE DISK FORMAT '$backup_dir/db_%d_df_t%t_s%s_p%p_level${backup_level}';" >> $cmdfile_name
# echo "ALLOCATE CHANNEL ch4 DEVICE TYPE DISK FORMAT '$backup_dir/db_%d_df_t%t_s%s_p%p_level${backup_level}';" >> $cmdfile_name
  ...
  echo "RELEASE CHANNEL ch2;" >> $cmdfile_name
# echo "RELEASE CHANNEL ch3;" >> $cmdfile_name
# echo "RELEASE CHANNEL ch4;" >> $cmdfile_name
  echo "}" >> $cmdfile_name
fi
