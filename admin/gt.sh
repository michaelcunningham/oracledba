...
spool ${derived_dir}/user_scripts.sql
...
spool ${derived_dir}/migrate_security.sql

prompt spool ${derived_dir}/migrate_security.lst
...
from	dba_role_privs
where	grantee not in ( 'SYS','SYSTEM','DBSNMP','STARADMIN','DMCYCLE','DBLINK_USER','PERFSTAT','TDCGLOBAL',
			'OUTLN','MDSYS','OPCO','ORDSYS','CTXSYS','ANONYMOUS','EXFSYS','DMSYS','WMSYS','XDB',
			'ORDPLUGINS','SI_INFORMTN_SCHEMA','OLAPSYS','DIP','TSMSYS')
...
from	dba_sys_privs
where	grantee not in ( 'SYS','SYSTEM','DBSNMP','STARADMIN','DMCYCLE','DBLINK_USER','PERFSTAT','TDCGLOBAL',
			'OUTLN','MDSYS','ORDSYS','CTXSYS','ANONYMOUS','EXFSYS','DMSYS','WMSYS','XDB',
			'ORDPLUGINS','OPCO','SI_INFORMTN_SCHEMA','OLAPSYS','DIP','TSMSYS')
and	grantee not like 'ORA\$%'
and	grantee not like 'NOVA%'
and	grantee not like 'VISTA___'
and	grantee not like 'VISTA_USER___'
...
prompt
prompt Running the ${derived_dir}/migrate_security.sql script
prompt
--start ${derived_dir}/migrate_security.sql
...
set termout off
set feedback off
spool ${derived_dir}/transfer_security_tables.sql

prompt
prompt spool ${derived_dir}/transfer_security_tables.lst
prompt
...
--start ${derived_dir}/transfer_security_tables.sql

set termout off
set feedback off

spool ${derived_dir}/update_role_privs.sql
...
--start ${derived_dir}/update_role_privs.sql
