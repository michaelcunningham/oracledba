...
sql_file=${log_dir}/${tns}_${username}_sequence_ddl.sql

syspwd=admin123

#
# Extract the ddl code for all sequences
#
...
--
-- Need a script to extract the code for all sequences for a user.
--
declare
	s_sql	clob;
begin
	...
	loop
		select dbms_metadata.get_ddl( 'SEQUENCE', r.sequence_name, '$username' ) into s_sql from dual;
