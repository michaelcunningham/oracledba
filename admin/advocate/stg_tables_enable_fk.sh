...
log_file=$advocate_log_dir/stg_tables_enchildfk_${ORACLE_SID}.log
stg_tables_list_file=$advocate_dir/stg_tables_list.txt
stg_tables_enchildfk_file=$advocate_dir/stg_tables_enchildfk_.sql

> $stg_tables_enchildfk_file

stg_tables=`cat $stg_tables_list_file`
for this_stg_table in $stg_tables
do
  echo "@/dba/admin/advocate/enchildfk "$this_stg_table >> $stg_tables_enchildfk_file
done
