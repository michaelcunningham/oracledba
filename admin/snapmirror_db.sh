...
echo >> $email_file
echo "${ORACLE_SID} snapmirror script started at:            `date`." >> $email_file
...
rm -f /${ORACLE_SID}arch/arch/*

####################################################################################################
#
# Copy the archive log files necessary for recovery
#
####################################################################################################
echo "${ORACLE_SID} get archive log files started at:        `date`." >> $email_file
...
echo "Exporting temp archive log filesystem."
rsh ${source_filer} exportfs -p rw,root=$server_name /vol/${source_arch_clone}

echo
echo
echo "Mounting filesystem "/tmp/${tmpdoc_mount}
sudo mount ${source_filer}:/vol/${source_arch_clone} /tmp/${tmpdoc_mount}

mkdir -p /${ORACLE_SID}arch/arch

source_archive_log_files=/tmp/${tmpdoc_mount}/arch/${SOURCE_SID}_archive_log_files.dat
archive_log_files=/${ORACLE_SID}arch/arch/${SOURCE_SID}_archive_log_files.dat
# echo $source_archive_log_files
# echo $archive_log_files
# echo
# echo

cp $source_archive_log_files $archive_log_files

archive_log_file_list=`cat $archive_log_files | awk '{sub("'/${SOURCE_SID}arch'","'/tmp/${tmpdoc_mount}'");print $1}'`

for this_archive_log_file in $archive_log_file_list
do
  if [ -s $this_archive_log_file ]
  then
    echo "      Copying $this_archive_log_file to /${ORACLE_SID}arch/arch"
    cp -p $this_archive_log_file /${ORACLE_SID}arch/arch
  fi
done

sudo umount /tmp/${tmpdoc_mount}
rmdir /tmp/${tmpdoc_mount}
rsh ${source_filer} exportfs -u /vol/${source_arch_clone}

echo "${ORACLE_SID} get archive log files completed at:      `date`." >> $email_file
...
#
####################################################################################################
echo "${ORACLE_SID} snapmirror initialize data started at:   `date`." >> $email_file
...
echo "${ORACLE_SID} snapmirror initialize data completed at: `date`." >> $email_file
...
/dba/admin/shrink_data_vol.sh ${ORACLE_SID} 0.7
...
echo "${ORACLE_SID} snapmirror script completed at:          `date`." >> $email_file

echo "" >> $email_file
echo "" >> $email_file
echo 'This report created by : '$0' '$* >> $email_file

mail -s "${ORACLE_SID} snapmirror report" `cat /dba/admin/dba_team` < $email_file
