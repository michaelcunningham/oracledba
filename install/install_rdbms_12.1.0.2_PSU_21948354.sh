...
log_file=/mnt/dba/install/logs/$(hostname -s)_install_rdbms_12.1.0.2_$(date +%a).log
>$log_file
...
#echo "TESTING if smon is running";
#ps -elf|grep -v grep |grep smon | grep -v ASM
#if [ "$?" -eq 0 ]
#then
#  echo "FOUND SMON.  PLEASE VALIDATE THAT YOU REALLY WANT TO RUN ON THIS SERVER."
#  exit 1
#fi
...
echo "        ######################################################################"
echo
#echo "        Installing Oracle Database Enterprise Edition 12.1.0.2"
echo
...
echo ">>>>>>>>>> /mnt/oracle_downloads/tarball-12.1.0.2/rdbms_12.1.0.2_PSU_21948354_binary.tar" | tee -a $log_file
echo | tee -a $log_file
mkdir -p /u01/app/oracle/product
cd /u01/app/oracle/product

tar xvf /mnt/oracle_downloads/tarball-12.1.0.2/rdbms_12.1.0.2_PSU_21948354_binary.tar | tee -a $log_file
chgrp dba /u01/app/oracle/product/12.1.0.2/dbhome_1/bin/oracle
chmod 6751 /u01/app/oracle/product/12.1.0.2/dbhome_1/bin/oracle

export ORACLE_BASE=/u01/app/oracle
export ORACLE_HOME=/u01/app/oracle/product/12.1.0.2/dbhome_1
export LD_LIBRARY_PATH=$ORACLE_HOME/jdbc/lib;$ORACLE_HOME/lib 

# Register RDBMS with the oracle inventory
echo | tee -a $log_file
echo ">>>>>>>>>> Registering RDBMS with the oracle inventory" | tee -a $log_file
echo | tee -a $log_file
$ORACLE_HOME/oui/bin/runInstaller -silent -clone ORACLE_HOME=$ORACLE_HOME ORACLE_HOME_NAME="OraRDBMS_12102" ORACLE_BASE=$ORACLE_BASE | tee -a $log_file
