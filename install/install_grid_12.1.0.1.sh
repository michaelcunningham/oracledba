...
log_file=/mnt/dba/install/logs/$(hostname -s)_install_grid_12.1.0.1_$(date +%a).log
>$log_file
...
echo "        ######################################################################"
echo
echo "        Installing Grid Control 12.1.0.1"
echo
...
echo ">>>>>>>>>> /mnt/oracle_downloads/tarball-12.1.0.1/grid_12.1.0.1_binary.tar" | tee -a $log_file
echo | tee -a $log_file
mkdir -p /u01/app
cd /u01/app
tar xvf /mnt/oracle_downloads/tarball-12.1.0.1/grid_12.1.0.1_binary.tar | tee -a $log_file
...
echo | tee -a $log_file
$ORACLE_HOME/perl/bin/perl $ORACLE_HOME/clone/bin/clone.pl -silent ORACLE_BASE=$ORACLE_BASE ORACLE_HOME=$ORACLE_HOME ORACLE_HOME_NAME=Ora12c_gridhome1 INVENTORY_LOCATION=/u01/app/oraInventory OSDBA_GROUP=dba OSASM_GROUP=dba OSOPER_GROUP=dba | tee -a $log_file
...
sudo /u01/app/12.1.0.1/grid/root.sh

# Configure Grid Infrastructure
echo | tee -a $log_file
echo ">>>>>>>>>> Configure Grid Infrastructure" | tee -a $log_file
echo | tee -a $log_file
sudo /u01/app/12.1.0.1/grid/perl/bin/perl -I/u01/app/12.1.0.1/grid/perl/lib -I/u01/app/12.1.0.1/grid/crs/install /u01/app/12.1.0.1/grid/crs/install/roothas.pl | tee -a $log_file
