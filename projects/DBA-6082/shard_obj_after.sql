...
select shard,object_type, count(object_name) "Obj count not like D_"
 from sys.objects_count_shards_after
 where object_type not in ('TRIGGER','INDEX PARTITION','TABLE PARTITION','INDEX')
...
select object_type,count(*) "Invalid count not like D_"
 from sys.objects_count_shards_after
 where status !='VALID'
 and object_name not like 'D%'
...
select object_type,count(*) "Correct obj count not like D_"
 from sys.objects_count_shards_after 
